...
@section('title', 'Ubah Dokumen Penerimaan Konsinyasi - ' . $receival->reference)
...
@section('content')
    <div class="container-fluid" x-data="consignmentReceivalEditForm()">
        @if ($errors->any())
        ...
        @if($receival->status === 'REJECTED')
            <div class="alert alert-warning">
                <strong>Catatan Penolakan Sebelumnya:</strong> {{ $receival->rejection_reason }}
            </div>
        @endif

        <form action="{{ route('consignments.receivals.update', $receival->id) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="form-row">
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="supplier_id" id="supplier_id" class="form-control consignment-supplier-select" required>
                                ...
                                <option value="{{ $receival->supplier_id }}" selected>{{ $receival->supplier->supplier_name }}</option>
                            </select>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="date">Tanggal Dokumen <span class="text-danger">*</span></label>
                            <input type="date" name="date" id="date" class="form-control" value="{{ old('date', $receival->date->format('Y-m-d')) }}" required>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="supplier_delivery_reference">No. Surat Jalan / Pengiriman Supplier</label>
                            <input type="text" name="supplier_delivery_reference" id="supplier_delivery_reference" class="form-control" value="{{ old('supplier_delivery_reference', $receival->supplier_delivery_reference) }}" placeholder="Contoh: SJ-2026/08/001">
                        </div>
                        <div class="col-12 mb-3">
                            <label for="note">Catatan</label>
                            <textarea name="note" id="note" class="form-control" rows="2" placeholder="Catatan opsional">{{ old('note', $receival->note) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-end mb-4">
                ...
                <button type="submit" class="btn btn-primary"><i class="bi bi-save"></i> Perbarui Draf</button>
            </div>
        </form>
    </div>
@endsection
...
<script>
    ...
    function consignmentReceivalEditForm() {
        return {
            lines: @json($receival->lines->values()->map(function ($l, $index) {
                return [
                    'key' => $index + 1,
                    'product_id' => $l->product_id,
                    'product_text' => trim($l->product_name . ($l->product_code ? ' (' . $l->product_code . ')' : '')),
                    'quantity' => (float) $l->quantity,
                    'unit_cost' => (float) $l->unit_cost,
                    'tax_id' => $l->tax_id ?? '',
                    'subtotal_cost' => (float) $l->subtotal_cost,
                ];
            })),
            nextLineKey: {{ $receival->lines->count() + 1 }},
            ...
            initProductSelect(element, line) {
                ...
                if (line.product_id && line.product_text) {
                    select.append(new Option(line.product_text, String(line.product_id), true, true));
                }
