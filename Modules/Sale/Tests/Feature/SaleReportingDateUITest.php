...
class SaleReportingDateUITest extends TestCase
{
    use RefreshDatabase;
    ...
    public function setUp(): void
    {
        ...
        // The detail view calls can('sales.reporting-date.override'); Spatie throws
    ...
    public function test_effective_date_displays_on_sale_detail_when_override_exists()
    {
        ...
        $this->service->setOverride($sale, $overrideDate, 'Test Override', $this->user);
    ...
    public function test_audit_history_displays_on_detail_page()
    {
        ...
        $date1 = now()->addDays(5);
        $audit1 = $this->service->setOverride($sale, $date1, 'First Override', $this->user);
    ...
    public function test_audit_history_shows_original_date()
    {
        ...
        $overrideDate = now()->addDays(5);
        $this->service->setOverride($sale, $overrideDate, 'Test', $this->user);
    ...
    public function test_audit_history_shows_null_override_as_safe_placeholder()
    {
        ...
        $this->service->setOverride($sale, now()->addDays(5), 'Set', $this->user);
        $this->service->clearOverride($sale, 'Clear', $this->user);
    ...
    public function test_detail_page_eager_loads_audit_relationships()
    {
        ...
        $sale->refresh();
        $this->service->setOverride($sale, now()->addDays(10), 'Second', $actor2);
        ...
        $viewSale = null;
        View::composer('sale::show', function ($view) use (&$viewSale) {
            $viewSale = $view->getData()['sale'] ?? null;
        });

        $response = $this->get(route('sales.show', $sale));

        $this->assertEquals(200, $response->status());
        $this->assertNotNull($viewSale, 'sale::show was not rendered with a sale.');
        $this->assertTrue(
            $viewSale->relationLoaded('reportingDateAudits'),
            'reportingDateAudits was not eager-loaded on the sale passed to the view.'
        );

        $audits = $viewSale->getRelation('reportingDateAudits');
    ...
    /**
     * The list is rendered by the SaleTable Livewire component, not inlined into
    ...
    public function test_sale_list_displays_effective_date_when_override_exists()
    {
        ...
        Livewire::test(SaleTable::class, ['settingId' => $this->setting->id])
            ->assertSee($overrideDate->format('d M Y'))
            ->assertDontSee($saleWithOverride->date->format('d M Y'));
    }

    public function test_sale_list_displays_original_date_when_no_override()
    {
        $saleNoOverride = $this->createSale();

        Livewire::test(SaleTable::class, ['settingId' => $this->setting->id])
            ->assertSee($saleNoOverride->date->format('d M Y'));
    }
}
