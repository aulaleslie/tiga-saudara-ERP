<?php

namespace Modules\Sale\Tests\Feature;

use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Modules\People\Entities\Customer;
use Modules\Product\Entities\Category;
use Modules\Product\Entities\Product;
use Modules\Product\Entities\ProductSerialNumber;
use Modules\Product\Entities\ProductStock;
use Modules\Purchase\Entities\PaymentTerm;
use Modules\Sale\Entities\Sale;
use Modules\Sale\Entities\SaleBundleItem;
use Modules\Sale\Entities\SaleDetails;
use Modules\Setting\Entities\Location;
use Modules\Setting\Entities\Setting;
use Modules\Setting\Entities\Tax;
use Modules\Setting\Entities\Unit;
use Spatie\Permission\Models\Permission;
use Spatie\Permission\PermissionRegistrar;
use Tests\TestCase;

class SalesDispatchBundleComponentSerialRegressionTest extends TestCase
{
    use RefreshDatabase;

    protected function setUp(): void
    {
        parent::setUp();
        $this->app->make(PermissionRegistrar::class)->forgetCachedPermissions();
    }

    public function test_normal_sale_dispatches_bundle_component_with_serials(): void
    {
        $setting = Setting::create([
            'company_name' => 'Test Company',
            'company_email' => 'company@example.com',
            'company_phone' => '1234567890',
            'default_currency_id' => 1,
            'default_currency_position' => 'prefix',
            'notification_email' => 'notify@example.com',
            'footer_text' => 'Footer',
            'company_address' => '123 Testing Lane',
            'is_pkp' => true,
        ]);

        $paymentTerm = PaymentTerm::create(['name' => 'Net 30', 'longevity' => 30]);
        $customer = Customer::factory()->create(['setting_id' => $setting->id, 'payment_term_id' => $paymentTerm->id]);

        Permission::firstOrCreate(['name' => 'sales.access']);
        Permission::firstOrCreate(['name' => 'sales.dispatch']);
        Permission::firstOrCreate(['name' => 'salesDispatches.approval']);

        $user = User::factory()->create();
        $user->givePermissionTo(['sales.access', 'sales.dispatch', 'salesDispatches.approval']);

        $category = Category::create([
            'category_name' => 'Category',
            'category_code' => 'CAT',
            'setting_id' => $setting->id,
            'created_by' => $user->id,
        ]);
        $unit = Unit::create([
            'name' => 'Unit',
            'short_name' => 'U',
            'operator' => '*',
            'operation_value' => 1,
            'setting_id' => $setting->id,
        ]);

        $location = Location::create([
            'name' => 'Warehouse Main',
            'setting_id' => $setting->id,
        ]);

        $tax = Tax::create(['name' => 'PPN 11%', 'value' => 11]);

        $bundleParent = Product::create([
            'product_name' => 'Gaming PC Bundle',
            'product_code' => 'BND-PC',
            'setting_id' => $setting->id,
            'product_quantity' => 1,
            'product_cost' => 5000000,
            'product_price' => 10000000,
            'category_id' => $category->id,
            'product_unit' => $unit->id,
            'stock_managed' => true,
            'serial_number_required' => false,
        ]);

        $component = Product::create([
            'product_name' => 'RTX 4090 GPU',
            'product_code' => 'GPU-4090',
            'setting_id' => $setting->id,
            'product_quantity' => 5,
            'product_cost' => 4000000,
            'product_price' => 8000000,
            'category_id' => $category->id,
            'product_unit' => $unit->id,
            'stock_managed' => true,
            'serial_number_required' => true,
        ]);

        ProductStock::create([
            'product_id' => $component->id,
            'location_id' => $location->id,
            'quantity' => 5,
            'quantity_tax' => 5,
            'quantity_non_tax' => 0,
            'broken_quantity' => 0,
            'broken_quantity_tax' => 0,
            'broken_quantity_non_tax' => 0,
        ]);

        $serial = ProductSerialNumber::create([
            'product_id' => $component->id,
            'serial_number' => 'SN-GPU-4090-001',
            'location_id' => $location->id,
            'status' => 'ACTIVE',
            'tax_id' => $tax->id,
        ]);

        $sale = Sale::create([
            'customer_id' => $customer->id,
            'customer_name' => $customer->customer_name,
            'total_amount' => 11100000,
            'paid_amount' => 0,
            'due_amount' => 11100000,
            'tax_amount' => 1100000,
            'tax_percentage' => 11,
            'discount_amount' => 0,
            'discount_percentage' => 0,
            'shipping_amount' => 0,
            'status' => Sale::STATUS_APPROVED,
            'payment_status' => 'Unpaid',
            'payment_method' => 'cash',
            'payment_term_id' => $paymentTerm->id,
            'setting_id' => $setting->id,
            'is_tax_included' => false,
            'reference' => 'SO-BUNDLE-SERIAL',
            'date' => now()->toDateString(),
            'due_date' => now()->addDays(7)->toDateString(),
        ]);

        $detail = SaleDetails::create([
            'sale_id' => $sale->id,
            'product_id' => $bundleParent->id,
            'product_name' => $bundleParent->product_name,
            'product_code' => $bundleParent->product_code,
            'quantity' => 1,
            'price' => 10000000,
            'unit_price' => 10000000,
            'sub_total' => 10000000,
            'product_discount_amount' => 0,
            'product_discount_type' => 'fixed',
            'product_tax_amount' => 1100000,
            'tax_id' => $tax->id,
        ]);

        $bundleId = 55;
        SaleBundleItem::create([
            'sale_id' => $sale->id,
            'sale_detail_id' => $detail->id,
            'product_id' => $component->id,
            'bundle_id' => $bundleId,
            'bundle_item_id' => 1,
            'name' => $component->product_name,
            'price' => 0,
            'quantity' => 1,
            'sub_total' => 0,
        ]);

        $compositeKey = $component->id . '-' . $tax->id . '-' . $bundleId;

        $payload = [
            'dispatch_date' => now()->toDateString(),
            'dispatchedQuantities' => [$compositeKey => 1],
            'selectedSerialNumbers' => [$compositeKey => [$serial->serial_number]],
            'serialNumberLocations' => [$compositeKey => [$serial->serial_number => $location->id]],
            'acknowledge_lifecycle_warning' => true,
        ];

        $response = $this->actingAs($user)
            ->withSession(['setting_id' => $setting->id])
            ->post(route('sales.storeDispatch', $sale), $payload);

        $response->assertRedirect();
        $response->assertSessionHasNoErrors();

        $this->assertDatabaseHas('dispatch_details', [
            'sale_id' => $sale->id,
            'product_id' => $component->id,
            'bundle_id' => $bundleId,
            'tax_id' => $tax->id,
            'dispatched_quantity' => 1,
            'location_id' => $location->id,
            'serial_numbers' => json_encode([$serial->serial_number]),
        ]);
    }
}
