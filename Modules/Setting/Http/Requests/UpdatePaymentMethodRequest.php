<?php

namespace Modules\Setting\Http\Requests;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Support\Facades\Gate;
use Modules\Setting\Entities\PaymentMethod;

class UpdatePaymentMethodRequest extends FormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     *
     * @return bool
     */
    public function authorize()
    {
        return Gate::allows('paymentMethods.edit');
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array
     */
    public function rules()
    {
        $paymentMethodId = $this->route('payment_method')->id;

        return [
            'name' => 'required|string|max:255|unique:payment_methods,name,' . $paymentMethodId,
            'coa_id' => 'required|exists:chart_of_accounts,id',
            'requires_reference' => 'nullable|boolean',
            'is_cash' => [
                'nullable',
                'boolean',
                function ($attribute, $value, $fail) use ($paymentMethodId) {
                    if ($value && PaymentMethod::where('is_cash', true)->where('id', '!=', $paymentMethodId)->exists()) {
                        $fail(__('A cash payment method already exists. You must disable the existing one before designating another.'));
                    }
                },
            ],
        ];
    }
}
