...
@section('breadcrumb')
    <ol class="breadcrumb border-0 m-0">
        ...
        <li class="breadcrumb-item"><a href="{{ route('expenses.imports.index') }}">Upload</a></li>
...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div>
                <a href="{{ route('expenses.imports.index') }}" class="btn btn-secondary me-2">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-2">
                        ...
                        <div class="h4 text-warning">{{ $batch->skipped_count }}</div>
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="mt-3">
                        <div class="progress" style="height: 25px;">
                            @php
                                ...
                                $skippedPct = $batch->total_rows > 0 ? round(($batch->skipped_count / $batch->total_rows) * 100) : 0;
                            ...
                            <div class="progress-bar bg-warning" style="width: {{ $skippedPct }}%">
                                {{ $skippedPct }}% Dilewati
                            </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                {{-- Filter Form --}}
                <form action="{{ route('expenses.imports.show', $batch) }}" method="GET" class="mb-3">
                    <div class="row g-2">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-4">
                            <input type="text" name="search" class="form-control" placeholder="Cari Error, Kategori..." value="{{ request('search') }}">
                        </div>
                        <div class="col-md-2">
                        ...
                        <div class="col-md-2">
                            @if(request()->hasAny(['status', 'search']))
                                <a href="{{ route('expenses.imports.show', $batch) }}" class="btn btn-outline-secondary w-100">Reset</a>
                            @endif
                        </div>
                    </div>
                </form>
                        ...
                        <tbody>
                            @forelse($rows as $row)
                                <tr>
                                    ...
                                    <td>{{ $row->raw_json['nomor'] ?? '-' }}</td>
                                    <td>{{ Str::limit($row->raw_json['kategori'] ?? '-', 20) }}</td>
                                    <td>{{ number_format((float) ($row->raw_json['jumlah'] ?? 0), 0, ',', '.') }}</td>
                                    ...
                                    <td>
                                        @if($row->expense_id)
                                            <a href="{{ route('expenses.show', $row->expense_id) }}" class="btn btn-sm btn-outline-primary">
                                                #{{ $row->expense_id }}
                                            </a>
                                        @else
                                            -
