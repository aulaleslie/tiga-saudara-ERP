...
class ReturnReceiptComparatorService
{
    /**
    ...
    public function requirePolicy(Transfer $transfer, TransferMovement $receiptMovement): TransferRoutePolicy
    {
        ...
        if ((int) $policy->origin_location_id !== (int) $receiptMovement->destination_location_id ||
            (int) $policy->destination_location_id !== (int) $receiptMovement->origin_location_id ||
    ...
    /**
     * Compare submitted return receipt movement against its exact approved source return dispatch movement.
     *
     * @param TransferMovement $receiptMovement (Type: RETURN_RECEIPT)
     * @param TransferMovement|null $sourceDispatchMovement (Type: RETURN_DISPATCH, Status: APPROVED)
     * @return array{
     *     matches: bool,
     *     summary: array{
    ...
    public function compare(TransferMovement $receiptMovement, ?TransferMovement $sourceDispatchMovement = null): array
    {
        ...
        if (!$sourceDispatchMovement) {
            throw new RuntimeException("Source return dispatch movement is missing.");
        }

        if ($sourceDispatchMovement->type !== TransferMovement::TYPE_RETURN_DISPATCH || $sourceDispatchMovement->status !== TransferMovement::STATUS_APPROVED) {
            throw new RuntimeException("Source movement must be an APPROVED RETURN_DISPATCH.");
        }

        if ($receiptMovement->return_batch_id !== $sourceDispatchMovement->return_batch_id) {
            throw new RuntimeException("Receipt return_batch_id [{$receiptMovement->return_batch_id}] does not match source dispatch return_batch_id [{$sourceDispatchMovement->return_batch_id}].");
        }
        ...
        foreach ($allProductIds as $prodId) {
            ...
            if (!$expected) {
                ...
                if ($comp < 0) {
                    ...
                    $expSerialCount = count($expectedSerials);
                    $cntSerialCount = count($countedSerials);
                    ...
                    $qtyMatchesExpSerials = bccomp($expectedQty, (string) $expSerialCount, 4) === 0;
                    $qtyMatchesCntSerials = bccomp($countedQty, (string) $cntSerialCount, 4) === 0;

                    if (!empty($missingSerials) || !empty($unexpectedSerials) || !$qtyMatchesExpSerials || !$qtyMatchesCntSerials || $hasIdMismatch) {
