...
class ForwardReceiptProjectionService
{
    ...
    /**
     * Build preparation projection for a forward receipt movement draft.
     ...
     * It NEVER exposes dispatch manifest, expected products/quantities/serials, allocation, or destination stock.
     */
    ...
    /**
     * Build approval review projection for a pending forward receipt movement according to permissions.
     */
    public function getApprovalProjection(Transfer $transfer, TransferMovement $movement, bool $canViewSystemStock): array
    {
        ...
        if (!$canViewSystemStock) {
            return [
                ...
                'message'         => $comparison['matches']
                    ? 'Hasil penerimaan fisik sesuai dengan pengiriman.'
                    : 'Terdapat ketidaksesuaian antara penerimaan fisik dan pengiriman.',
            ];
        }
        ...
        // Include destination stock and dispatched bucket provenance for privileged approvers
        ...
        foreach ($details as $prodId => &$detailRow) {
            $destStock = $destStocks->get($prodId);
            $sourceLine = $sourceLines->firstWhere('product_id', $prodId);

            $detailRow['destination_stock'] = [
                'quantity'         => (int) ($destStock->quantity ?? 0),
                'quantity_tax'     => (int) ($destStock->quantity_tax ?? 0),
                'quantity_non_tax' => (int) ($destStock->quantity_non_tax ?? 0),
                'broken_tax'       => (int) ($destStock->broken_quantity_tax ?? 0),
                'broken_non_tax'   => (int) ($destStock->broken_quantity_non_tax ?? 0),
            ];
