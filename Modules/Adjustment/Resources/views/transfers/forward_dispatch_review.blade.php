...
@section('title', 'Tinjauan Persetujuan Pengiriman')
...
@section('breadcrumb')
    <ol class="breadcrumb border-0 m-0">
        ...
        <li class="breadcrumb-item active">Tinjauan Persetujuan</li>
    </ol>
@endsection
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div>
                            <h4 class="card-title mb-0">Tinjauan Persetujuan Pengiriman (V2 Forward Dispatch)</h4>
                    ...
                    <div class="card-body">
                        @if($projection['matches'])
                            <div class="alert alert-success" role="alert">
                                <h5 class="alert-heading">✓ Perhitungan Fisik Cocok</h5>
                                <p class="mb-0">Hasil perhitungan fisik sesuai dengan permintaan transfer yang telah disetujui.</p>
                            </div>
                        @else
                            <div class="alert alert-danger" role="alert">
                                <h5 class="alert-heading">⚠ Terdapat Ketidaksesuaian</h5>
                                <p class="mb-0">Hasil perhitungan fisik tidak sesuai dengan permintaan transfer.</p>
                            </div>
                        @endif

                        @if($canViewSystemStock && !empty($projection['details']))
                            <h5 class="mt-4 mb-3">Detail Perbandingan Item</h5>
                                ...
                                <table class="table table-bordered table-striped">
                                    <thead class="bg-light">
                                        <tr>
                                            <th>Produk</th>
                                            <th>Kode</th>
                                            <th class="text-center">Permintaan Transfer</th>
                                            <th class="text-center">Perhitungan Aktual</th>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                            ...
                            <div class="d-flex gap-2">
                                @if($movement->status === \Modules\Adjustment\Entities\TransferMovement::STATUS_PENDING)
                                    <form action="{{ route('transfers.movements.reject', ['transfer' => $transfer->id, 'movement' => $movement->id]) }}" method="POST" class="d-inline mr-2" onsubmit="return promptRejectReason(this);">
                                        @csrf
                                        <input type="hidden" name="reason" class="reject-reason" value="">
                                        <button type="submit" class="btn btn-danger">
                                            <i class="bi bi-x-circle mr-1"></i> Tolak Pengiriman
                                        </button>
                                    </form>
                                    @if($projection['matches'])
                                        <form action="{{ route('transfers.movements.approve', ['transfer' => $transfer->id, 'movement' => $movement->id]) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-success">
                                                <i class="bi bi-check-circle mr-1"></i> Setujui Pengiriman & Kurangi Stok
                                            </button>
                                        </form>
                                    @endif
                                @endif
...
function promptRejectReason(form) {
    var reason = prompt("Masukkan alasan penolakan pengiriman:");
