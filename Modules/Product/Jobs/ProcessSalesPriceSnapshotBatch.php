...
class ProcessSalesPriceSnapshotBatch implements ShouldQueue
{
    ...
    private function stageRowsFromXlsx(ProductImportBatch $batch): ?int
    {
        ...
        foreach ($rowIterator as $row) {
            ...
            if ($isFirstRow) {
                ...
                $required = ['Name*', 'SellPrice']; // ProductCode is optional
                $missing = [];
    ...
    private function processRows(ProductImportBatch $batch): void
    {
        ...
        \Modules\Product\Entities\ProductImportRow::where('batch_id', $batch->id)
            ->whereNull('status')
            ->chunkById(500, function ($rows) use ($batch, $settingsMap, $settingsMapNames, $markerResolver, $codeMap, $exactNameMap, $normalizedNameMap, &$validRows) {
                foreach ($rows as $row) {
                    ...
                    // 1. Resolve owner setting ID
                    $companyName = $markerResolver->resolveOwnerCompanyName($rawName);
                    $settingId = $settingsMap[$companyName] ?? null;

                    if (!$settingId) {
                        $this->recordFailure($batch, $row, "Unmapped owner marker for company: {$companyName}", 'error', $meta);
                        continue;
                    }
    ...
    private function applyMutations(ProductImportBatch $batch, array $validRows): void
    {
        ...
        foreach ($groups as $key => $groupRows) {
            ...
            if ($conflict) {
                foreach ($groupRows as $row) {
                    $this->recordFailure($batch, $row, 'Conflicting duplicate prices for the same target.', 'error', $row->result_metadata ?? []);
                }
                continue;
            }
            ...
            try {
                ...
                // Ensure purchase prices are preserved if new
                ...
                // First row is 'imported', subsequent exact duplicates are 'duplicate'
                $isFirst = true;
                foreach ($groupRows as $row) {
