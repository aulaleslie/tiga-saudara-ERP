...
@section('title', 'POS Reconciliation')

@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h1 class="h3 mb-0 text-gray-800">POS Reconciliation</h1>
            <div class="d-flex align-items-center">
                <div class="me-2 d-flex align-items-center">
                    <label for="date-from" class="me-2 mb-0 whitespace-nowrap">From</label>
                    <input type="date" id="date-from" class="form-control form-control-sm" value="{{ date('Y-m-d') }}">
                </div>
                <div class="me-3 d-flex align-items-center">
                    <label for="date-to" class="me-2 mb-0 whitespace-nowrap">To</label>
        ...
        <div class="card shadow mb-4">
            <div class="card-header border-bottom-0">
                <h6 class="m-0 font-weight-bold text-primary">Session Cross-Reference & Reconciliation</h6>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-striped mb-0">
                        <thead class="table-light">
                            <tr>
                                <th>Session / Terminal</th>
                                <th>Lifecycle</th>
                                <th class="text-end">Opening</th>
                                <th class="text-end">Expected</th>
                                <th class="text-end">Counted</th>
                                <th class="text-end">Variance</th>
                                <th class="text-end">Checkout Total</th>
                                <th class="text-end">Posted Sales</th>
                                <th class="text-end">Posted Pay</th>
                                <th class="text-center">Status</th>
                            </tr>
                        </thead>
                        <tbody id="reconciliation-tbody">
                            <tr><td colspan="10" class="text-center text-muted py-4">Click Load to retrieve data.</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const loadData = async () => {
        ...
        btnRefresh.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i> Loading';
        ...
        try {
            ...
            if (data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="10" class="text-center text-muted py-4">No closed sessions found in date range.</td></tr>';
                return;
            }
            ...
            tbody.innerHTML = data.map(row => {
                ...
                const badge = row.has_mismatch 
                    ? `<span class="badge bg-danger" title="${row.mismatch_details}">Mismatch</span>` 
                ...
                return `
                    <tr class="${trClass}">
                        <td>
                            ...
                            <div class="small text-muted">By: ${row.cashier_name}</div>
                        </td>
                        <td class="small">
                            O: ${formatDate(row.opened_at)}<br>
                            C: ${formatDate(row.closed_at)}
                        </td>
                        <td class="text-end">${formatCurrency(row.opening_float)}</td>
                        <td class="text-end fw-bold">${formatCurrency(row.expected_cash)}<br>
                            <span class="badge bg-secondary">Drops: ${formatCurrency(row.safe_drop_total)}</span>
                        </td>
            ...
            btnRefresh.innerHTML = '<i class="fas fa-sync-alt"></i> Load';
        }
    };
