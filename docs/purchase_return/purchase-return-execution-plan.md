...
9) Ticket 9: Hide purchase price on purchase create.

Rationale: Build the core return data model and UI first, then add serial logic and validations, then lifecycle controls. Permissions and purchase-create visibility can proceed in parallel but are placed after core flow to minimize rework.

## 2. Parallelizable Tasks
- Ticket 1 can run in parallel with Tickets 2–7 (auth layer is independent).
- Ticket 9 can run in parallel with all return tickets (separate module).
...
Milestone D: Access + purchase create privacy
- Tickets 1 and 9 complete.
- Result: Access control enforced; purchase create hides price.
...
- Risk: Price hidden on create but exposed through cached UI state.
...
- API tests:
  ...
  - Purchase create API ignores price on create responses.
- UI tests:
