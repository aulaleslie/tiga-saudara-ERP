...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('products', function (Blueprint $table) {
            if (!$this->indexExists('products', 'idx_products_product_name')) {
                $table->index('product_name', 'idx_products_product_name');
            }
            if (!$this->indexExists('products', 'idx_products_product_code')) {
            ...
            if (!$this->indexExists('products', 'idx_products_serial_req_id')) {
                $table->index(['serial_number_required', 'id'], 'idx_products_serial_req_id');
            }
        });

        // Product stock indexes for location-based SUM queries
        if (!$this->indexExists('product_stocks', 'idx_product_stocks_location_product')) {
        ...
        Schema::table('product_unit_conversions', function (Blueprint $table) {
            if (!$this->indexExists('product_unit_conversions', 'idx_product_unit_conversions_barcode')) {
                $table->index('barcode', 'idx_product_unit_conversions_barcode');
            }
            if (!$this->indexExists('product_unit_conversions', 'idx_product_unit_conversions_product_id')) {
        ...
        Schema::table('product_serial_numbers', function (Blueprint $table) {
            if (!$this->indexExists('product_serial_numbers', 'idx_psn_location_product_dispatch_broken')) {
                $table->index(['location_id', 'product_id', 'dispatch_detail_id', 'is_broken'], 'idx_psn_location_product_dispatch_broken');
            }
            if (!$this->indexExists('product_serial_numbers', 'idx_psn_serial_number')) {
                $table->index('serial_number', 'idx_psn_serial_number');
            }
        });

        // Purchase details index for "purchased products" check
        if (!$this->indexExists('purchase_details', 'idx_purchase_details_product_id')) {
        ...
        if (!$this->indexExists('product_prices', 'idx_product_prices_product_setting')) {
        ...
        if (!$this->indexExists('product_bundles', 'idx_product_bundles_parent_product_id')) {
