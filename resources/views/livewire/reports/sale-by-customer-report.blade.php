        ...
        <div class="offcanvas-body p-3" style="overflow-y: auto; flex-grow: 1;">
            ...
            <div class="mb-3">
                ...
                <select wire:model="sortField" class="form-control form-control-sm mb-2">
                    ...
                    <option value="customer_total">Total Penjualan Customer</option>
                </select>
            ...
            @if($filterTriggered)
                ...
                @forelse($sales as $row)
                    @php
                        ...
                        $nextCustomerId = $nextRow ? $nextRow->sale->customer_id : null;
                    ...
                    <tr @if($mapped['is_tax_row']) class="text-muted bg-light" @endif>
                        <td class="ps-4">{{ date('d/m/Y', strtotime($row->sale->effective_date)) }}</td>
                        <td>{{ $mapped['Tipe transaksi'] }}</td>
                        <td>
                            @can('sales.show')
                                <a href="{{ route('sales.show', $row->sale->id) }}" class="text-primary fw-bold">
                                    {{ $mapped['No. transaksi'] }}
                                </a>
                            @else
                ...
                @empty
                    <tr>
                        <td colspan="10" class="text-center py-4 text-muted">
                            ...
                            Tidak ada data penjualan per customer yang sesuai dengan filter ini.
                        </td>
                    </tr>
                @endforelse
                @if($sales->count() > 0)
