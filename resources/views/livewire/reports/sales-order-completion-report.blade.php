<div x-data="{ showDrawer: false, isApplying: false }" x-init="$watch('showDrawer', value => { if(!value && !isApplying) { $wire.cancelFilters(); } isApplying = false; })">
    ...
    <div class="d-flex justify-content-between align-items-center mt-4 mb-2">
        <h5 class="mb-0">Penyelesaian Pemesanan Penjualan</h5>
    ...
    <div class="table-responsive shadow-sm rounded">
        <table class="table table-hover table-bordered mb-0" style="font-size: 0.85rem; min-width: 1000px;">
            ...
            <tr>
                ...
                <th wire:click="sortBy('total_amount')" class="text-end" style="cursor:pointer; white-space:nowrap">Jumlah Pesanan {!! $this->sortIcon('total_amount') !!}</th>
            ...
            @if($filterTriggered)
                ...
                @forelse($sales as $sale)
                    @php
                        $mapped = $mapRow($sale);
                        // Convert formatted back to float for totals if needed, or simply sum them from the query service
                        // The mapper provides unformatted floats for amounts as standard in other reports
                    ...
                    <tr>
                        ...
                        <td class="text-end">{{ number_format((float)$mapped['Jumlah Pesanan'], 0, ',', '.') }}</td>
                        <td>{{ $mapped['Status Pesanan'] }}</td>
                ...
                @if($sales->count() > 0)
                    <tr class="table-secondary fw-bold fs-6">
                        ...
                        <td class="text-end text-primary">{{ number_format((float)$grandTotalPesanan, 0, ',', '.') }}</td>
                        <td></td>
                        <td class="text-end text-primary">{{ number_format((float)$grandTotalPengiriman, 0, ',', '.') }}</td>
                        <td class="text-end text-primary">{{ number_format((float)$grandTotalFaktur, 0, ',', '.') }}</td>
                        <td class="text-end text-primary">{{ number_format((float)$grandTotalPembayaran, 0, ',', '.') }}</td>
                    </tr>
                @endif
            @else
                <tr>
