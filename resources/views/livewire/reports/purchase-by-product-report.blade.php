<div x-data="{ showDrawer: false, isApplying: false }" x-init="$watch('showDrawer', value => { if(!value && !isApplying) { $wire.cancelFilters(); } isApplying = false; })">
    ...
    <div class="d-flex flex-wrap gap-2 mb-3 align-items-end">
        @if($canViewGlobal)
        <div>
            <label class="form-label small">Cabang / Cabang Bisnis</label>
            <select wire:model.live="scopeSettingId" class="form-control">
        ...
        <div class="offcanvas-body p-3" style="overflow-y: auto; flex-grow: 1;">
                    ...
                    @if(count($productIds) > 0)
                        <div class="mt-2 d-flex flex-wrap gap-1">
                            @foreach($productIds as $id)
                                <span class="badge rounded-pill bg-warning text-dark d-inline-flex align-items-center px-2 py-1">
                                    {{ $productLabels[$id] ?? 'ID:'.$id }}
                                    <button type="button" class="btn-close btn-close-dark ms-2" style="font-size: 0.5rem" wire:click="removeProduct({{ $id }})"></button>
                                </span>
                            @endforeach
                        </div>
                    @endif
            ...
            <div class="mb-3">
                ...
                <select wire:model="sortField" class="form-control form-control-sm mb-2">
                    <option value="product_code">Kode Produk / SKU</option>
                    <option value="product_name">Nama Produk</option>
                    <option value="purchase_quantity">Qty Pembelian</option>
                    <option value="return_quantity">Qty Retur</option>
                    <option value="purchase_value">Nilai Pembelian</option>
                    <option value="average_purchase_value">Nilai Pembelian Rata-rata</option>
                </select>
    ...
    <div class="table-responsive shadow-sm rounded mt-3">
        <table class="table table-hover table-bordered mb-0" style="font-size: 0.8rem; min-width: 1200px;">
            ...
            <tr>
                <th style="white-space:nowrap">Kode Produk / SKU</th>
                <th style="white-space:nowrap">Nama Produk</th>
                <th class="text-end" style="white-space:nowrap">Qty Pembelian</th>
                <th class="text-end" style="white-space:nowrap">Qty Retur</th>
                <th style="white-space:nowrap">Satuan</th>
                <th class="text-end" style="white-space:nowrap">Nilai Pembelian</th>
                <th class="text-end" style="white-space:nowrap">Nilai Retur</th>
                <th class="text-end" style="white-space:nowrap">Nilai Pembelian Rata-rata</th>
            </tr>
            </thead>
            ...
            @if($filterTriggered)
                @forelse($products as $row)
                    <tr>
                        ...
                        <td class="text-end">{{ number_format((float)$row->purchase_quantity, 2, ',', '.') }}</td>
                        <td class="text-end">{{ number_format((float)$row->return_quantity, 2, ',', '.') }}</td>
                        <td>{{ $row->unit_name }}</td>
                        <td class="text-end">{{ number_format((float)$row->purchase_value, 0, ',', '.') }}</td>
                        <td class="text-end text-danger">{{ number_format((float)$row->return_value, 0, ',', '.') }}</td>
                        <td class="text-end">{{ number_format((float)$row->average_purchase_value, 0, ',', '.') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="text-center py-4 text-muted">
                            <i class="bi bi-inbox fs-2 d-block mb-2"></i>
                            Tidak ada data pembelian per produk yang sesuai dengan filter ini.
                        </td>
                    </tr>
                @endforelse
                @if($products->count() > 0)
                    <tr class="table-secondary fw-bold fs-6">
                        ...
                        <td class="text-end text-primary">{{ number_format((float)$grandTotalPurchase, 0, ',', '.') }}</td>
