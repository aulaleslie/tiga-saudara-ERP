<div x-data="{ showDrawer: false, isApplying: false }" x-init="$watch('showDrawer', value => { if(!value && !isApplying) { $wire.cancelFilters(); } isApplying = false; })">
    ...
    <div class="d-flex flex-wrap gap-2 mb-3 align-items-end">
        ...
        <div>
            <label class="form-label small">As of Date (Hingga Tanggal)</label>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" :class="showDrawer ? 'show' : ''"
        ...
        <div class="offcanvas-body p-3" style="overflow-y: auto; flex-grow: 1;">
            ...
            {{-- Due Date Until --}}
            <div class="mb-3">
                <label class="form-label small">Jatuh Tempo Hingga</label>
                <input type="date" wire:model.defer="dueDateUntil" class="form-control">
            </div>
    ...
    <div class="table-responsive shadow-sm rounded mt-3">
        <table class="table table-hover table-bordered mb-0" style="font-size: 0.8rem; min-width: 1000px;">
            ...
            <tr>
                <th style="white-space:nowrap">Pelanggan / Tanggal</th>
                ...
                <th class="text-end" style="white-space:nowrap">Sisa Piutang</th>
            </tr>
            </thead>
            ...
            @if($filterTriggered)
                ...
                @forelse($sales as $row)
                    @php
                        ...
                        $mappedRows = \App\Services\Reports\CustomerReceivablesReportQueryService::mapRows($row);
                        $isFirstRow = $loop->first;
                        $isLastRow = $loop->last;
                        $nextRow = $isLastRow ? null : $sales[$loop->index + 1];
                        $nextCustomerId = $nextRow ? $nextRow->customer_id : null;

                        $isNewCustomer = $currentCustomerId !== $customerId;
                        if ($isLastRow) {
                            $isEndOfCustomer = !isset($nextPageFirstCustomerId) || $nextPageFirstCustomerId !== $customerId;
                        } else {
                            $isEndOfCustomer = $nextCustomerId !== $customerId;
                        }
                    ...
                    @if($isNewCustomer)
                        <tr class="table-active fw-bold">
                            <td colspan="7">
                                {{ $mappedRows['Pelanggan / Tanggal'] }}
                                @if($isFirstRow && $customerId === ($previousPageLastCustomerId ?? null))
                    ...
                    <tr>
                        ...
                        <td>{{ $mappedRows['Tipe transaksi'] }}</td>
                        <td>
                            @can('sales.show')
                                <a href="{{ route('sales.show', $row->id) }}" class="text-primary fw-bold">
                                    {{ $mappedRows['No. transaksi'] }}
                                </a>
                            @else
                                <strong>{{ $mappedRows['No. transaksi'] }}</strong>
                            @endcan
                        </td>
                        <td>{{ $mappedRows['Jatuh Tempo'] }}</td>
                        <td>{{ $mappedRows['Deskripsi'] }}</td>
                        <td class="text-end">{{ number_format($mappedRows['Jumlah'], 0, ',', '.') }}</td>
                        <td class="text-end text-primary fw-bold">{{ number_format($mappedRows['Sisa Piutang'], 0, ',', '.') }}</td>
                    </tr>
                    @if($isEndOfCustomer)
                        <tr class="table-light fw-bold">
                            <td class="ps-4" colspan="5">Subtotal</td>
                            <td class="text-end">{{ number_format($row->previous_running_jumlah + $mappedRows['Jumlah'], 0, ',', '.') }}</td>
                            <td class="text-end text-primary">{{ number_format($row->previous_running_total + $mappedRows['Sisa Piutang'], 0, ',', '.') }}</td>
                        </tr>
                    @endif
                @empty
                    <tr>
                ...
                @if($sales->count() > 0)
                    <tr class="table-secondary fw-bold fs-6">
                        <td class="ps-4" colspan="5">Total</td>
                        <td class="text-end">{{ number_format((float)$grandTotalJumlah, 0, ',', '.') }}</td>
                        <td class="text-end text-primary">{{ number_format((float)$grandTotal, 0, ',', '.') }}</td>
                    </tr>
                @endif
            @else
                <tr>
