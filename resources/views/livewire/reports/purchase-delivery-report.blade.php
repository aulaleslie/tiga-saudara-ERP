<div x-data="{ showDrawer: false, isApplying: false }" x-init="$watch('showDrawer', value => { if(!value && !isApplying) { $wire.cancelFilters(); } isApplying = false; })">
    ...
    <div class="table-responsive shadow-sm rounded mt-3">
        <table class="table table-hover table-bordered mb-0" style="font-size: 0.8rem; min-width: 1200px;">
            ...
            <tr>
                <th style="white-space:nowrap">Supplier & Kode produk / SKU</th>
            ...
            @if($filterTriggered)
                ...
                @forelse($deliveries as $index => $row)
                    @php
                        ...
                        $isNewSupplier = $currentSupplierId !== $row->supplier_id;
                        
                        if ($isLastRow) {
                            $isEndOfSupplier = !isset($nextPageFirstSupplierId) || $nextPageFirstSupplierId !== $row->supplier_id;
                        } else {
                            $isEndOfSupplier = $nextSupplierId !== $row->supplier_id;
                        }
                        
                        $currentSupplierId = $row->supplier_id;
                    @endphp
                    @if($isNewSupplier)
                        <tr class="table-active fw-bold">
                            <td colspan="5">
                                {{ $row->supplier_name ?? 'Tanpa Supplier' }}
                                @if($isFirstRow && $row->supplier_id === ($previousPageLastSupplierId ?? null))
                    ...
                    <tr>
                        ...
                        <td class="text-end fw-bold text-primary">{{ number_format((float)$row->delivered_amount, 2, ',', '.') }}</td>
                    </tr>
                    @if($isEndOfSupplier)
                        <tr class="table-light fw-bold">
                            <td class="ps-4">Subtotal</td>
                            <td colspan="3"></td>
                            <td class="text-end text-primary">{{ number_format((float)$row->current_running_total, 2, ',', '.') }}</td>
                        </tr>
                    @endif
                @empty
                    <tr>
                        <td colspan="5" class="text-center py-4 text-muted">
                            ...
                            Tidak ada data pengiriman pembelian yang sesuai dengan filter ini.
                        </td>
                    </tr>
                @endforelse
                @if($deliveries->count() > 0)
                    <tr class="table-secondary fw-bold fs-6">
                        ...
                        <td colspan="3"></td>
                        <td class="text-end text-primary">{{ number_format((float)$grandTotal, 2, ',', '.') }}</td>
                    </tr>
                @endif
            @else
                <tr>
