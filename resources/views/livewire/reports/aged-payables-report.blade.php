<div x-data="{ showDrawer: false, isApplying: false }" x-init="$watch('showDrawer', value => { if(!value && !isApplying) { $wire.cancelFilters(); } isApplying = false; })">
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" :class="showDrawer ? 'show' : ''"
        ...
        <div class="offcanvas-body p-3" style="overflow-y: auto; flex-grow: 1;">
            ...
            <div class="mb-3">
                <label class="form-label small">Dasar Usia</label>
                <select wire:model="agingBasis" class="form-control form-control-sm">
        ...
        <table class="table table-hover table-bordered mb-0" style="font-size: 0.8rem; min-width: 1000px;">
            ...
            @if($filterTriggered)
                @forelse($purchases as $row)
                    @php
                        $mappedRows = \App\Services\Reports\AgedPayablesReportQueryService::mapRows($row);
                    @endphp
                    <tr>
                        <td class="ps-4">{{ $mappedRows['Vendor'] }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center py-4 text-muted">
                            ...
                            Tidak ada data utang supplier yang sesuai dengan filter ini.
                        </td>
                    </tr>
                @endforelse
                @if($purchases->count() > 0 && $grandTotals)
                    <tr class="table-secondary fw-bold fs-6">
                        <td class="ps-4">Total Hutang</td>
