<div x-data="{ showDrawer: false, isApplying: false }" x-init="$watch('showDrawer', value => { if(!value && !isApplying) { $wire.cancelFilters(); } isApplying = false; })">
    ...
    <div class="table-responsive shadow-sm rounded mt-3">
        <table class="table table-hover table-bordered mb-0" style="font-size: 0.8rem; min-width: 1200px;">
            ...
            <tr>
                <th style="white-space:nowrap">Customer / Tanggal</th>
                <th style="white-space:nowrap">No. Pengiriman</th>
                <th style="white-space:nowrap">Kode / SKU</th>
                <th style="white-space:nowrap">Nama Produk</th>
                <th class="text-end" style="white-space:nowrap">Qty Dikirim</th>
                <th style="white-space:nowrap">Satuan</th>
                <th class="text-end" style="white-space:nowrap">Harga per unit</th>
                <th class="text-end" style="white-space:nowrap">Total Nominal</th>
            </tr>
            </thead>
            ...
            @if($filterTriggered)
                ...
                @endphp
                @forelse($deliveries as $index => $row)
                    @php
                        $isFirstRow = $loop->first;
                        $isLastRow = $loop->last;
                        $nextRow = $isLastRow ? null : $deliveries[$index + 1];
                        $nextCustomerId = $nextRow ? $nextRow->customer_id : null;

                        $isNewCustomer = $currentCustomerId !== $row->customer_id;
                        
                        if ($isLastRow) {
                            $isEndOfCustomer = !isset($nextPageFirstCustomerId) || $nextPageFirstCustomerId !== $row->customer_id;
                        } else {
                            $isEndOfCustomer = $nextCustomerId !== $row->customer_id;
                        }
                    ...
                    @if($isNewCustomer)
                        <tr class="table-active fw-bold">
                            <td colspan="8">
                                ...
                                @if($isFirstRow && $row->customer_id === ($previousPageLastCustomerId ?? null))
                    ...
                    @endif
                    <tr>
                        <td class="ps-4">{{ date('d/m/Y', strtotime($row->dispatch_date)) }}</td>
                        <td>{{ $row->dispatch_reference }}</td>
                        <td>{{ $row->product_code }}</td>
                        <td>{{ $row->product_name }}</td>
                        <td class="text-end">{{ number_format((float)$row->delivered_quantity, 2, ',', '.') }}</td>
                        <td>{{ $row->unit_name }}</td>
                        <td class="text-end">{{ number_format((float)$row->unit_amount, 0, ',', '.') }}</td>
                        <td class="text-end fw-bold text-primary">{{ number_format((float)$row->delivered_amount, 0, ',', '.') }}</td>
                    </tr>
                    @if($isEndOfCustomer)
                        <tr class="table-light fw-bold">
                            <td class="ps-4">Subtotal</td>
                            <td colspan="6"></td>
                            <td class="text-end text-primary">{{ number_format((float)$row->current_running_total, 0, ',', '.') }}</td>
                        </tr>
                    @endif
                @empty
                    <tr>
                        <td colspan="8" class="text-center py-4 text-muted">
                            ...
                            Tidak ada data pengiriman penjualan yang sesuai dengan filter ini.
                        </td>
                    </tr>
                @endforelse
    ...
    @if($filterTriggered && $deliveries instanceof \Illuminate\Pagination\LengthAwarePaginator)
        <div class="mt-3">
            {{ $deliveries->links() }}
        </div>
    @endif
