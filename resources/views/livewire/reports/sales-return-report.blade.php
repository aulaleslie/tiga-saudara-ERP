                    ...
                    <table class="table table-bordered table-striped text-center mb-0">
                        ...
                        @forelse($sale_returns as $sale_return)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $srStatusLabel = \Modules\SalesReturn\Entities\SaleReturn::statusLabel($sale_return->status);
                                        if ($sale_return->status === 'Pending') {
                                            $srStatusLabel = 'Menunggu';
                                        } elseif ($sale_return->status === 'Shipped') {
                                            $srStatusLabel = 'Dikirim';
                                        } elseif ($sale_return->status === 'Completed') {
                                            $srStatusLabel = 'Selesai';
                                        }
                                    @endphp
                                    @if ($sale_return->status == 'Pending')
                                ...
                                <td>
                                    @if (\App\Constants\PaymentStatus::matches($sale_return->payment_status, \App\Constants\PaymentStatus::PARTIAL))
                                        <span class="badge badge-warning">
                                            {{ \App\Constants\PaymentStatus::label($sale_return->payment_status) }}
                                        </span>
                                    @elseif (\App\Constants\PaymentStatus::matches($sale_return->payment_status, \App\Constants\PaymentStatus::PAID))
                                        <span class="badge badge-success">
                                            {{ \App\Constants\PaymentStatus::label($sale_return->payment_status) }}
                                        </span>
                                    @else
