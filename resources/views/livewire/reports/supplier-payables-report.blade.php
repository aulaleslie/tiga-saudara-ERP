            ...
            @if($filterTriggered)
                ...
                @forelse($purchases as $row)
                    @php
                        ...
                        $mappedRows = \App\Services\Reports\SupplierPayablesReportQueryService::mapRows($row);
                        $isFirstRow = $loop->first;
                        $isLastRow = $loop->last;
                        $nextRow = $isLastRow ? null : $purchases[$loop->index + 1];
                        $nextSupplierId = $nextRow ? $nextRow->supplier_id : null;
                    ...
                    <tr>
                        ...
                        <td>
                            @can('purchases.show')
                                <a href="{{ route('purchases.show', $row->id) }}" class="text-primary fw-bold">
                        ...
                        <td>{{ $mappedRows['Keterangan'] }}</td>
                        <td class="text-end">{{ number_format($mappedRows['Jumlah'], 0, ',', '.') }}</td>
                        <td class="text-end text-primary fw-bold">{{ number_format($mappedRows['Saldo'], 0, ',', '.') }}</td>
                    </tr>
                    @if($isEndOfSupplier)
                        <tr class="table-light fw-bold">
                            ...
                            <td class="text-end text-primary">{{ number_format($row->previous_running_total + $mappedRows['Saldo'], 0, ',', '.') }}</td>
                        </tr>
                    @endif
                @empty
                    <tr>
                        <td colspan="7" class="text-center py-4 text-muted">
                            ...
                            Tidak ada data hutang supplier yang sesuai dengan filter ini.
                        </td>
                    </tr>
                @endforelse
