<div x-data="{ showDrawer: false, isApplying: false }" x-init="$watch('showDrawer', value => { if(!value && !isApplying) { $wire.cancelFilters(); } isApplying = false; })">
            ...
            <div class="mb-3">
                ...
                <div class="position-relative">
                    <input type="text" wire:model.live.debounce.300ms="customerSearch"
                           class="form-control" placeholder="Cari Pelanggan (min 2 karakter)...">
                    @if(strlen($customerSearch) >= 2)
                        <div class="list-group position-absolute w-100 shadow-lg mt-1 bg-white text-dark" style="z-index: 1060; max-height: 250px; overflow-y: auto; border: 1px solid #dee2e6;">
                            @forelse($customerOptions as $option)
                                ...
                                @else
                                    <button type="button"
                                            wire:click="selectCustomer({{ $option['id'] }}, '{{ addslashes($option['customer_name']) }}')"
                            ...
                            @empty
                                <div class="list-group-item bg-white disabled small py-3 text-center text-muted">
                                    <i class="bi bi-search me-1"></i> Tidak ada pelanggan ditemukan
                                </div>
                            @endforelse
                        </div>
                    @endif
    ...
    @php
        ...
        $emptyColspan = $isHeaderMode ? 12 : 32;
    @endphp

    <div class="table-responsive shadow-sm rounded" wire:loading.class="opacity-50">
        <table class="table table-hover table-bordered mb-0" style="font-size: 0.8rem; min-width: {{ $isHeaderMode ? '1400px' : '2600px' }};">
            ...
            @if($isHeaderMode)
                <tr>
                    ...
                    <th wire:click="sortBy('customer_name')" style="cursor:pointer; white-space:nowrap">Nama Pelanggan {!! $this->sortIcon('customer_name') !!}</th>
            ...
            @if($filterTriggered)
                @forelse($sales as $row)
                    @php
                        $mapped = \App\Services\Reports\SaleReportQueryService::mapRow($row, $tableReportMode);
                        $sale = $tableReportMode === 'header' ? $row : $row->sale;
                ...
                @empty
                    <tr>
                        <td colspan="{{ $emptyColspan }}" class="text-center py-4 text-muted">
                            ...
                            Tidak ada data penjualan yang sesuai dengan filter ini.
                        </td>
                    </tr>
                @endforelse
            @else
