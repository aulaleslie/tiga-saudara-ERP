# inventory-remaining-stock-visibility Specification

## Purpose

Gates visibility of exact remaining-stock quantities behind a dedicated permission, independent of surface-access permissions like `pos.sell` or `pricePoints.access`, so that numeric stock data is only exposed to users explicitly authorized to see it while stock-state indicators (out-of-stock, service) remain visible to everyone.
