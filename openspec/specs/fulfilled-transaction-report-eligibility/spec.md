# fulfilled-transaction-report-eligibility Specification

## Purpose

Define one shared report eligibility policy for sales- and purchase-derived reports: sales are recognized only after full dispatch and purchases only after full receipt, persisted settlement-modified values are authoritative without separate return subtraction, fully returned documents are excluded once their return lifecycle is completed/archived, and the same eligibility and value-source rules apply consistently across on-screen rows, totals, pagination, setting scope, and exports. Delivery reports and order-completion reports keep their existing, distinct semantics.
