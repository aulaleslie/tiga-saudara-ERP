...
The system SHALL calculate balances as of a selected date for the active tenant setting.
...
- **THEN** the Neraca report includes only transactions for the active `setting_id`
...
The system SHALL allow authorized users to export the filtered Neraca report to XLSX using the same calculation output shown on screen.

#### Scenario: Export uses current filter
- **WHEN** the user exports the Neraca report after selecting an as-of date
- **THEN** the XLSX file uses the same as-of date and report rows as the on-screen report
