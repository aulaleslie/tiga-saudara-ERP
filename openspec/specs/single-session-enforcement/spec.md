# single-session-enforcement Specification

## Purpose
Ensure that only one active login per user account exists at a time across devices, without falsely destroying or invalidating legitimate concurrent requests from the same login (such as multiple tabs in the same browser).
