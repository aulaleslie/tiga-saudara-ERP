# pos-rounded-amount-consistency Specification

## Purpose

POS transaction 3393 saved a rounded grand total of Rp318,000 while its receipt and detail rows summed to Rp317,992 because rounded row amounts were lost or recomputed from unit prices instead of being persisted. This capability establishes a durable POS row-amount contract so that calculated row amounts and header totals are captured once and consumed identically by persistence, presentation, checkout, payments, reporting, and returns — the amount billed remains the amount settled, posted, reported, and used for returns — while preserving existing automatic-rounding eligibility, decimal unit prices, and manual pricing authority, and providing an explicit, auditable repair path for drafts affected before this fix.
