# current-product-name-display Specification

## Purpose

Ensure purchase, sales, and bundle transaction surfaces (interactive screens, printable commercial documents, and exports) display the current name of a linked product rather than the name persisted at transaction time, while product renaming never rewrites historical transaction-line snapshots and the persisted name remains a safe fallback when the linked product or its current name cannot be resolved. Name resolution for collections of lines is bounded (no per-line product query).
