...
The system SHALL expose escaped sale transaction header notes in Global Payment contexts, SHALL present list notes in a dedicated `Catatan` column immediately after `Ref` using compact and individually expandable presentation, and SHALL present payment candidates in entry-prioritized due-date order.

#### Scenario: Sale note is visible in the list note column
- **WHEN** an eligible sale is rendered in a standalone or customer-embedded Global Payment list
...
#### Scenario: Sale allocation note remains associated with document identity
- **WHEN** an eligible sale with a non-blank header note is rendered in the sales allocation form
- **THEN** the escaped note remains displayed with that sale's document identity
