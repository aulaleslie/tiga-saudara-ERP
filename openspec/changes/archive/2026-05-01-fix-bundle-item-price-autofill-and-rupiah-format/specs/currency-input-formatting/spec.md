...
#### Scenario: Bundle item informational price submits canonical numeric value
- **WHEN** user submits Product Bundle create/edit with `Item Paket` prices currently displayed as formatted currency
- **THEN** each submitted `items[*][informational_item_price]` value MUST be raw numeric
- **AND** backend numeric validation MUST parse it without locale-specific currency parsing
