...
- **AND** Penjualan shows Daftar Penjualan, Penjualan Per Customer, Piutang pelanggan, Usia piutang, Pengiriman penjualan, Penjualan per produk, and Penyelesaian pesanan penjualan (gated by `saleReports.access`) and Penjualan Global (gated by `saleReports.global.access`)
...
#### Scenario: Penyelesaian pesanan penjualan card is actionable

- **WHEN** a user with `saleReports.access` views the Penjualan tab
- **THEN** the Penyelesaian pesanan penjualan card is rendered as an actionable report link
- **AND** the Penyelesaian pesanan penjualan card does not show placeholder or unavailable-state treatment
