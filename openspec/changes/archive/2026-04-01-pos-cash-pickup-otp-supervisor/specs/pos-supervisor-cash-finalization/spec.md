...
**Note**: The supervisor authentication mechanism for the **cash pickup (safe drop)** flow within this capability changes from email+password to supervisor-ID+OTP. The finalization flow's own variance approval authentication (email+password in the finalization modal) is NOT changed by this modification — it remains as-is for now.
...
#### Scenario: Cash pickup uses OTP-based supervisor approval instead of email+password
- **WHEN** a cashier initiates a cash pickup (safe drop) via the POS sell page
- **THEN** Step 2 of the cash pickup modal SHALL present a searchable supervisor dropdown and TOTP code input instead of email and password fields
- **AND** the `POST /pos/sessions/{session}/pickup` endpoint SHALL accept `supervisor_id` (integer) and `otp_code` (string) instead of `supervisor_email` and `supervisor_password`
- **AND** the supervisor SHALL be verified via `TwoFactorService::verifyCode()` instead of `Hash::check` on password
