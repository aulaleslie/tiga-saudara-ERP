...
The system SHALL provide a dedicated Product module workflow for uploading an Accurate-style XLSX product export as a sales-price snapshot, separate from generic product creation and stock snapshot imports.
...
- **WHEN** an authorized user submits an XLSX workbook with `Name*` and `SellPrice` columns
...
- **WHEN** an authorized user submits a file that is not a readable XLSX workbook
- **THEN** the system SHALL reject the upload without creating price mutations
