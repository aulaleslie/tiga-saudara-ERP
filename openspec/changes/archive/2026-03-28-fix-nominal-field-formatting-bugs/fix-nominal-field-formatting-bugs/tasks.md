...
## 9. Deterministic RP Formatting Contract (New Direction)

- [x] 9.1 Define fixed product nominal format contract: prefix `RP `, thousands `.`, decimal `,`, precision `2`
- [x] 9.2 Remove product nominal display dependency on `settings()->currency` in create/edit/component paths
- [x] 9.3 Ensure parser/formatter logic is deterministic and not system-locale dependent (no `toLocaleString`/`Intl` for this flow)
- [x] 9.4 Fix focus/blur lifecycle so raw digits like `65000` cannot be reinterpreted as `0.65`
- [x] 9.5 Validate strict raw/display contract end-to-end
- [x] 9.6 Create page: input raw `50000` -> blur `RP 50.000,00` and hidden raw `50000`
- [x] 9.7 Edit page: load `RP 60.000,00` -> focus `60000` -> edit `65000` -> blur `RP 65.000,00`
- [x] 9.8 Decimal case: input raw `1234.56` -> blur `RP 1.234,56` and hidden raw `1234.56`
