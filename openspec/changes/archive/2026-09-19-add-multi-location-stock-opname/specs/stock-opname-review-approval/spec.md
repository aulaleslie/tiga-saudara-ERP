...
The system SHALL verify the action permission and eligibility of every selected location for show, edit, update, delete, submit, approve, and reject operations. Stock-opname location eligibility SHALL NOT depend on the user's current active setting. Product and location resolution SHALL reject inactive, nonexistent, duplicate, and consignment locations, and failure of any selected location SHALL reject the complete operation without exposing unauthorized stock or partially mutating the document.
...
- **WHEN** any selected location is inactive, deleted, consignment, or otherwise unauthorized at an authoritative lifecycle boundary
- **THEN** the complete operation is rejected without inventory mutation or disclosure from that location
