...
- **AND** Produk shows Mutasi Stok (gated by `stockMutationReports.access`), Mutasi Stok Global (gated by `stockMutationReports.global.access`), Valuasi Stok (gated by `inventoryValuationReports.access`), and Ringkasan persediaan barang (gated by `inventoryValuationReports.access`)
