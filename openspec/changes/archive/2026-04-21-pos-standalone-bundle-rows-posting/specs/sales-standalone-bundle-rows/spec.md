...
#### Scenario: POS-originated bundle row remains valid
- **WHEN** POS checkout posting persists a bundle row for a sale
- **THEN** Sales SHALL accept the row as valid regardless of whether `sale_detail_id` is linked or null
- **AND** Sales SHALL require deterministic standalone context fields when parent linkage is absent
...
#### Scenario: POS-created standalone row is consumable by Sales read paths
- **WHEN** a sale includes standalone bundle rows persisted from POS posting
- **THEN** Sales detail, dispatch aggregation, and return eligibility read paths SHALL consume those rows using standalone context fallback
- **AND** those read paths SHALL NOT depend on the existence of a parent `sale_details` row for those bundle rows
