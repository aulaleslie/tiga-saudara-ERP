...
- **AND** Pembelian shows Daftar Pembelian, Pembelian Per Supplier, and Utang supplier (gated by `purchaseReports.access`) and Pembelian Global (gated by `purchaseReports.global.access`)
...
#### Scenario: Utang supplier card is actionable

- **WHEN** a user with `purchaseReports.access` views the Pembelian tab
- **THEN** the Utang supplier card is rendered as an actionable report link
- **AND** the Utang supplier card does not show placeholder or unavailable-state treatment
