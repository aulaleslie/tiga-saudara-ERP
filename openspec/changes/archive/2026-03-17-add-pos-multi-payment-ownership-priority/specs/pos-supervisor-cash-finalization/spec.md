...
- **THEN** the modal SHALL display:
  ...
  - Expected cash breakdown: opening_float_total + cash-components-from-checkout-payments - safe_drops = expected_cash_total
...
- **THEN** expected_cash_total SHALL be calculated as: opening_float_total + sum(checkout_payment.amount_paid WHERE payment_method.is_cash = true) - sum(safe_drops with direction OUT)
- **AND** if a checkout contains both cash and non-cash methods, only the cash portion contributes to expected cash
