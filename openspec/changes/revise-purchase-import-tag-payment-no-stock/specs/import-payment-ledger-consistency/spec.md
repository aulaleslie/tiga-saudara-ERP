...
The purchase importer SHALL create active `purchase_payments` rows for future imported purchase documents whose resolved cash payment or non-cash deduction credit is greater than zero. CSV `Status Hari Ini` and CSV `Total` are the authoritative source for payment status and settlement derivation. Calculated line totals SHALL create item details and complement the CSV document state; when they differ from CSV `Total`, the importer SHALL reconcile generated purchase `total_amount` to the CSV `Total` before allocating settlement.
...
- **AND** the purchase `paid_amount` MUST equal the CSV `Total`
...
- **AND** the purchase `due_amount` MUST equal the CSV `Total`
...
- **AND** the purchase `paid_amount` plus `due_amount` MUST reconcile to the CSV `Total`
...
The purchase and sales importers SHALL resolve imported paid amount from `Pembayaran` when it is present and non-blank, and SHALL fall back to document total minus preferred outstanding balance only when `Pembayaran` is blank or missing. For purchase imports, CSV `Status Hari Ini` drives the payment status mapping; paid and due amounts are derived against CSV `Total`, and generated purchase totals are reconciled to that CSV `Total` before settlement components are persisted.
...
- **AND** `Pembayaran` is blank, zero, stale, or otherwise does not reconcile with the CSV `Total`
- **THEN** the purchase importer MUST resolve paid amount to the CSV `Total`
...
- **AND** payment fields do not reconcile with the CSV `Total`
- **THEN** the purchase importer MUST resolve paid amount to `0.00`
- **AND** the purchase importer MUST resolve outstanding balance to the CSV `Total`

#### Scenario: Mismatched line total reconciles to CSV Total
- **WHEN** a purchase CSV invoice group has a source CSV `Total` that differs from the calculated line total
- **THEN** the purchase importer MUST reconcile the generated purchase `total_amount` to the CSV `Total`
- **AND** the importer MUST NOT create a negative `due_amount` or over-settle any owner group
...
- **AND** the importer MUST validate or derive outstanding balance against the authoritative document total

#### Scenario: Missing Pembayaran falls back to outstanding balance
- **WHEN** an invoice group has no usable `Pembayaran` value
- **THEN** the importer MUST calculate resolved imported paid amount as authoritative document total minus preferred outstanding balance unless a purchase `Status Hari Ini` rule explicitly overrides it
...
- **THEN** the importer MUST use `Sisa Tagihan Hari Ini` as the preferred outstanding balance when it reconciles with the authoritative document total and resolved settlement components

#### Scenario: Sisa Tagihan is fallback
- **WHEN** an invoice group does not include a usable `Sisa Tagihan Hari Ini` value
- **THEN** the importer MUST use `Sisa Tagihan` as the preferred outstanding balance when it is present and reconciles with the authoritative document total and resolved settlement components
