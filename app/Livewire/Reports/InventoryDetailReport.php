...
class InventoryDetailReport extends Component
{
    ...
    public function exportExcel(InventoryDetailReportQueryService $queryService)
    {
        abort_unless(auth()->user()->can('stockMutationReports.access'), 403);
        ...
        $result = $queryService->getDetail($filterData, $this->settingId, PHP_INT_MAX, 1);

        $dateStart = Carbon::parse($filterData->tanggalAwal)->format('d-m-Y');
        $dateEnd = Carbon::parse($filterData->tanggalAkhir)->format('d-m-Y');
        $fileName = sprintf('detail-persediaan-barang_%s_sd_%s.xlsx', $dateStart, $dateEnd);

        return Excel::download(
            new \App\Exports\InventoryDetailReportExport($result['allRows'], $filterData, false),
            $fileName
        );
    }

    public function exportCsv(InventoryDetailReportQueryService $queryService)
    {
        abort_unless(auth()->user()->can('stockMutationReports.access'), 403);
        ...
        $dateStart = Carbon::parse($filterData->tanggalAwal)->format('d-m-Y');
        $dateEnd = Carbon::parse($filterData->tanggalAkhir)->format('d-m-Y');
        $fileName = sprintf('detail-persediaan-barang_%s_sd_%s.csv', $dateStart, $dateEnd);

        return Excel::download(
            new \App\Exports\InventoryDetailReportExport($result['allRows'], $filterData, true),
            $fileName,
    ...
    public function render(InventoryDetailReportQueryService $queryService)
    {
        ...
        return view('livewire.reports.inventory-detail-report', [
            'paginator' => $paginator,
        ]);
    }
}
