...
class PurchaseOrderCompletionReportExport implements FromCollection, WithHeadings, WithMapping, WithEvents, WithColumnFormatting
{
    public function __construct(
        private Builder $query,
        private PurchaseOrderCompletionReportFilterData $filter,
    ...
    public function collection()
    {
        ...
        foreach ($data as $purchase) {
            $mapped = PurchaseOrderCompletionReportQueryService::mapRow($purchase);
            $item = new \stdClass();
            $item->is_total_row = false;
            $item->data = $mapped;
            $rows->push($item);

            $grandTotalPemesanan += $mapped['Jumlah Pemesanan'];
        ...
        if (!$this->isCsv && $rows->count() > 0) {
            ...
            $totalRow->data = [
                ...
                'No. Pemesanan' => '',
                'Jumlah Pemesanan' => $grandTotalPemesanan,
                'Status Pemesanan' => '',
    ...
    {
        return PurchaseOrderCompletionReportQueryService::headings();
    }
    ...
    public function map($row): array
    {
        if ($this->isCsv) {
            return [
                ...
                number_format((float) $row->data['Jumlah Pemesanan'], 2, '.', ''),
                $row->data['Status Pemesanan'],
        ...
        return [
            $row->data['Tanggal Pemesanan'],
            $row->data['No. Pemesanan'],
            (float) $row->data['Jumlah Pemesanan'],
            $row->data['Status Pemesanan'],
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) use ($startDate, $endDate, $companyName) {
                ...
                // Title
                $sheet->setCellValue('A2', 'purchase_order_completion');
